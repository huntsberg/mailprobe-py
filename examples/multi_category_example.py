...
def basic_multi_category_example():
    ...
    with MultiCategoryFilter(categories) as classifier:
        ...
        for email_content, expected in test_emails:
            ...
            print(
                f"Expected: {expected:12} | Classified: {result.category:12} | Confidence: {result.confidence:.3f}"
            )
        ...
        for category, category_stats in stats.items():
            print(
                f"{category:12}: {category_stats['word_count']} words, "
    ...
    try:
        ...
        with FolderBasedClassifier(temp_dir) as classifier:
            ...
            for category, category_stats in stats.items():
                ...
                print(
                    f"  {category:12}: {email_count} emails, {category_stats['word_count']} words"
                )
...
def advanced_multi_category_example():
    ...
    with MultiCategoryFilter(categories) as classifier:
        ...
        for email in ambiguous_emails:
            ...
            print(
                f"Classified as: {result.category} (confidence: {result.confidence:.3f})"
            )
...
def email_routing_example():
    ...
    class EmailRouter:
        ...
        def train_from_user_actions(self, email_content, user_folder):
            ...
            self.classifier.train_category(
                user_folder, [email_content], force_update=True
            )
    ...
    try:
        ...
        for email in test_emails:
            ...
            print(
                f"  '{subject}' → {routing['folder']} (confidence: {routing['confidence']:.3f})"
            )
