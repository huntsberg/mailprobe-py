...
class CategoryResult:
    ...
    def __str__(self) -> str:
        return f"{self.category} (prob={self.probability:.3f}, conf={self.confidence:.3f})"
...
class MultiCategoryFilter:
    ...
    def __init__(self, 
        ...
        self.database_path = Path(database_path) if database_path else Path.home() / ".mailprobe-py-multi"
    ...
    def train_category(self, 
                      ...
                      force_update: bool = False) -> CategoryTrainingResult:
        """
    ...
    def classify(self, 
                ...
                return_all_scores: bool = False) -> CategoryResult:
        """
    ...
    def get_all_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {category: self.get_category_stats(category) for category in self.categories}
    
    def cleanup_category(self, category: str, max_count: int = 2, max_age_days: int = 7) -> int:
        """
    ...
    def cleanup_all_categories(self, max_count: int = 2, max_age_days: int = 7) -> Dict[str, int]:
        """
    ...
    def _get_message(self, message_source: Union[str, Path, EmailMessage]) -> EmailMessage:
    ...
    def _get_messages_from_source(self, source: Union[List[str], List[Path], List[EmailMessage], str, Path]) -> List[EmailMessage]:
...
class FolderBasedClassifier:
    ...
    def _discover_categories(self) -> List[str]:
        ...
        for item in self.base_path.iterdir():
            if (item.is_dir() and 
                item.name not in self.exclude_folders and 
                not item.name.startswith('.')):
                categories.append(item.name)
        
        return sorted(categories)
    
    def train_from_folders(self, force_update: bool = False) -> Dict[str, CategoryTrainingResult]:
        """
    ...
    def move_email_to_folder(self, 
                           email_file: Union[str, Path], 
                           category: str) -> Path:
        """
    ...
    def classify_and_move(self, 
                         email_file: Union[str, Path],
                         confidence_threshold: float = 0.5) -> Tuple[CategoryResult, Optional[Path]]:
        """
...
def classify_into_categories(email_content: str, 
                           categories: List[str],
                           database_path: Optional[Union[str, Path]] = None) -> CategoryResult:
    """
...
def train_from_folder_structure(base_path: Union[str, Path],
                               database_path: Optional[Union[str, Path]] = None) -> Dict[str, CategoryTrainingResult]:
    """
