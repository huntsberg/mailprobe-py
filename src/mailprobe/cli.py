...
@click.option('-d', '--database-path', type=click.Path(), 
              help='Database directory path')
@click.option('-c', '--create-db-dir', is_flag=True,
...
@click.option('-l', '--spam-threshold', type=float,
              help='Spam probability threshold (0.0-1.0)')
@click.option('-C', '--min-word-count', type=int,
              help='Minimum word count for calculated probability')
@click.option('-p', '--max-phrase-terms', type=int,
              help='Maximum number of words per phrase')
@click.option('-w', '--terms-for-score', type=int,
              help='Number of terms to use for scoring')
@click.option('-r', '--max-word-repeats', type=int,
              help='Maximum repeats of same word in scoring')
@click.option('-s', '--cache-size', type=int,
              help='Database cache size')
@click.option('-h', '--no-remove-html', is_flag=True,
              help='Do not remove HTML tags')
@click.option('-x', '--extend-top-terms', is_flag=True,
              help='Extend top terms array')
@click.option('-X', '--aggressive-scoring', is_flag=True,
              help='Use aggressive scoring settings')
@click.option('-o', '--option', multiple=True,
              help='Special options (graham, conservative, etc.)')
@click.pass_context
def cli(ctx, **kwargs):
...
def receive(ctx, files):
    ...
    with MailFilter(config.get_database_path(), config.to_filter_config()) as spam_filter:
        reader = EmailMessageReader()
        
        if files:
            # Process files
            for file_path in files:
                _process_messages_receive(spam_filter, reader, Path(file_path), config.verbose)
        else:
            # Process stdin
...
def train(ctx, files):
    ...
    with MailFilter(config.get_database_path(), config.to_filter_config()) as spam_filter:
        ...
        if files:
            # Process files
            for file_path in files:
                _process_messages_train(spam_filter, reader, Path(file_path), config.verbose)
        else:
            # Process stdin
            message = reader.read_from_stdin()
            score = spam_filter.score_message(message)
...
@cli.command()
@click.argument('files', nargs=-1, type=click.Path(exists=True))
@click.option('-T', '--show-terms', is_flag=True,
              help='Show top terms used for scoring')
@click.pass_context
def score(ctx, files, show_terms):
    ...
    with MailFilter(config.get_database_path(), config.to_filter_config()) as spam_filter:
        ...
        if files:
            # Process files
            for file_path in files:
                _process_messages_score(spam_filter, reader, Path(file_path), show_terms)
        else:
            # Process stdin
            message = reader.read_from_stdin()
            score = spam_filter.score_message(message)
...
def good(ctx, files):
    ...
    with MailFilter(config.get_database_path(), config.to_filter_config()) as spam_filter:
        ...
        if files:
            for file_path in files:
                count += _process_messages_classify(spam_filter, reader, Path(file_path), 
...
@cli.command()
@click.option('--max-count', type=int, default=2,
              help='Maximum count for words to be removed')
...
def dump(ctx, output_format):
    ...
    with MailFilter(config.get_database_path(), config.to_filter_config()) as spam_filter:
        ...
        if output_format == 'csv':
            ...
            import json
            data = [{"term": term, "good_count": good, "spam_count": spam} 
...
def _process_messages_receive(spam_filter: MailFilter, reader: EmailMessageReader,
                            file_path: Path, verbose: bool) -> int:
...
def _process_messages_train(spam_filter: MailFilter, reader: EmailMessageReader,
                          file_path: Path, verbose: bool) -> int:
...
def _process_messages_score(spam_filter: MailFilter, reader: EmailMessageReader,
                          file_path: Path, show_terms: bool) -> int:
...
def _process_messages_classify(spam_filter: MailFilter, reader: EmailMessageReader,
                             file_path: Path, is_spam: bool, verbose: bool) -> int:
